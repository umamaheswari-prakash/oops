...
class Stall(Ice_cream):
    ...
    def __init__(self, shape, flavour, topping, quantity):
        super().__init__(shape, flavour, topping)
    ...
    def total_cost(self):
        if self.flavour == "chocolate":
            return ("The updated ice_cream price is:%s" % (Stall.shape_cost(self) + Stall.flavour_cost(self) + Ice_cream.adding_topping(self)))
        else:
